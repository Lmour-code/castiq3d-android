...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Button>(R.id.btnCalibrateMode).setOnClickListener {
            activeTool = Tool.CALIBRATE
            calibrationTapPoints.clear()
            updateOverlay()
            toast("Calibration: tap 2 points on the model spanning a known distance.")
        }
        ...
        findViewById<Button>(R.id.btnPlaceLandmark).setOnClickListener {
            activeTool = Tool.LANDMARK
            toast("Tap the model to place: ${landmarkTypes[spinnerLandmarkType.selectedItemPosition]}")
        }
    ...
    private fun handlePick(origin: Vec3, dir: Vec3) {
        ...
        if (hit == null) {
            runOnUiThread { toast("No surface hit — tap directly on the model.") }
            return
        }
        runOnUiThread {
            when (activeTool) {
                ...
                Tool.NONE -> { /* rotate-only tap, ignore */ }
            }
        }
    }
